...
import com.marcuschu.aicodegenerate.ai.tools.FileWriteTool;
...
public class AiCodeGeneratorServiceFluxFactory {
    ...
    private AiCodeGeneratorFluxService createAiCodeGeneratorFluxService(long appId, CodeGenTypeEnum codeGenType) {
        ...
        return switch (codeGenType) {
            // Vue 项目生成使用推理模型
            case VUE_PROJECT -> AiServices.builder(AiCodeGeneratorFluxService.class)
                    ...
                    .build();
            // HTML 和多文件生成使用默认模型
            case HTML, MULTI_FILE -> AiServices.builder(AiCodeGeneratorFluxService.class)
                    .chatModel(chatModel)
